...
(function ($) {
  var BF = {
    ...
    init: function () {
      ...
      this.$elem
        .find("#bf-faq-n-troubleshooting")
        .one("click", this.insertFaqFrame);
      ...
      $("a[href]").on(
        ...
        function (e) {
          ...
          if (
            ...
            !confirm(this.lang.confirm_unsaved_changes)
          ) {
            e.preventDefault();
            return false;
          }
    ...
    initTabs: function () {
      ...
      this.$elem.find(".tabs").each(function () {
        ...
        if (!$tabs.filter(".selected").length)
    ...
    initAdvacedSettings: function () {
      $("[data-adv-group]").each(function () {
        ...
        $(this).prop(
          "disabled",
          $(this).is(".disabled") ||
            $('[data-disable-adv="' + group + '"]').val() !== "1",
        );
      });
      this.$elem.on("change", "[data-disable-adv]", function () {
        var group = $(this).data("disable-adv"),
          val = $(this).val();
        $('[data-adv-group="' + group + '"]').each(function () {
          $(this).prop(
            "disabled",
            $(this).is(".disabled") ||
              !(val === "1" || $(this).data("for-val") === val),
          );
        });
      });
    },
    ...
    initAutocomplete: function () {
      this.$elem.find(".bf-autocomplete").each(
        function (i, v) {
          var lookup = [],
          ...
          $(v).autocomplete({
            ...
            onSelect: function (suggestion) {
              var data = $.extend(
                {},
                this[$(v).data("lookup")][suggestion.data],
              );
      ...
      this.$elem.find(".bf-attr-autocomplete").each(
        function (i, v) {
          var lookup = [];
          for (var i in this.attributes) {
            var name =
              this.attributes[i].group + " / " + this.attributes[i].name;
            lookup.push({ value: name, data: i });
          }
          $(v).autocomplete({
            lookup: lookup,
    ...
    initCustomFilterSettings: function () {
      ...
      for (var i in this.settings.attributes) {
        ...
        data.name =
          "<b>" +
          this.attributes[i].group +
          "</b> / " +
          this.attributes[i].name;
        this.addRow(
          $("#custom-attr-settings"),
          $("#custom-attr-setting-template").html(),
          i,
          data,
        );
      }
      ...
      for (var i in this.settings.options) {
        ...
        this.addRow(
          $("#custom-opt-settings"),
          $("#custom-opt-setting-template").html(),
          i,
          data,
        );
      }
      ...
      for (var i in this.settings.filters) {
        ...
        this.addRow(
          ...
          $("#custom-filter-setting-template").html(),
          i,
          data,
        );
      }
    ...
    initSelectAll: function () {
      this.$elem.find("[data-select-all]").click(function () {
        ...
        $(this)
          .closest('[data-select-all-group="' + group + '"]')
          .find(cls)
          .click();
        return false;
      });
    ...
    initCategoryTab: function () {
      ...
      $("[data-unselect-all]").on("click", function () {
        $($(this).data("unselect-all"))
          .find(".bf-row")
          .removeClass("bf-selected");
      });
      ...
      this.$elem.find(".bf-move-left").on("click", function () {
        $listEnabled.append(
          $listDisabled.find(".bf-row.bf-selected").removeClass("bf-selected"),
        );
        return false;
      });
      this.$elem.find(".bf-move-right").on("click", function () {
        $listDisabled.append(
          $listEnabled.find(".bf-row.bf-selected").removeClass("bf-selected"),
        );
        return false;
      });
      ...
      $("#bf-layout-id").change(function () {
        $('[data-target="#bf-categories"]').toggleClass(
          "hidden",
          BF.categoryLayouts.indexOf($(this).val()) === -1,
        );
      });
    },
    ...
    initOrderingTab: function () {
      ...
      $("#bf-attr-values .bf-save-btn").on(
        "click",
        this.saveAttributeValuesOrder,
      );
      ...
      $.ajax({
        ...
        success: function (json) {
          ...
          for (var lang in json) {
            var $group = $(
              '<div class="bf-row-group lang-' + lang + '"></div>',
            );
            for (var i in json[lang]) {
              var $row = $('<div class="bf-row"></div>');
              $row
                .data("valueId", json[lang][i].attribute_value_id)
                .text(json[lang][i].value);
              $group.append($row);
            }
            $list.append($group);
    ...
    hideEmptyTables: function ($tbl) {
      ...
      $tbl.each(function () {
        ...
        if (isEmpty) {
          $(this).after(
            '<div class="alert alert-info">' + BF.lang.empty_table + "</div>",
          );
        } else {
      ...
      for (var name in settings) {
        ...
        if (field.length) {
          var type = field.eq(0).attr("type")
            ? field.eq(0).attr("type").toLowerCase()
            : "";
    ...
    addRow: function ($tbl, tplStr, id, vals) {
      ...
      if (vals.name) {
        ...
        $tpl
          .find("[data-bf-role=control]")
          .on("change", this.gridOptionViewMode);
      }
      if (vals.collapsed) {
    ...
    validateForm: function () {
      ...
      if (!layoutSelected && isInstance) {
        success = false;
        $(".breadcrumb").after(
          '<div class="alert alert-warning bf-validation-msg">' +
            BF.lang.error_layout_not_set +
            "</div>",
        );
      }
