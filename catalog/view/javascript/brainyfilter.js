  ...
  if (typeof BrainyFilter === "undefined") {
    var BrainyFilter = {
      ...
      init: function () {
        ...
        if (
          (this.requestCount || this.requestPrice) &&
          this.separateCountRequest
        ) {
          this.getTotalByAttr();
      ...
      initSliders: function () {
        $(".bf-slider-container").each(function () {
          var $slider = $(this),
            ...
            $s = $("[data-slider-group=" + id + "]")
              .not($slider)
              .closest(".bf-attr-filter"),
          ...
          $slider.slider({
            ...
            slide: function (e, ui) {
              ...
              minLbl
                .attr("value", labels[ui.values[0]].n)
                .val(labels[ui.values[0]].n);
              maxLbl
                .attr("value", labels[ui.values[1]].n)
                .val(labels[ui.values[1]].n);
              ...
              $s.find(".bf-slider-container").slider(
                "option",
                "values",
                ui.values,
              );
          ...
          var changeSlider = function () {
            var $slider = $(this)
                .closest(".bf-attr-filter")
                .find(".bf-slider-container"),
              id = $slider.data("slider-group"),
              val = $(this).val(),
              target = $(this).hasClass("bf-slider-text-inp-min")
                ? "min"
                : "max",
              fVal = -1;
            for (var i = 0; i < labels.length; i++) {
      ...
      addCross: function ($obj) {
        ...
        $("input[data-filterid=" + fid + "]").each(function () {
          ...
          if ($obj[0].checked) {
            ...
            var cross = $(
              '<span class="bf-cross" data-filterid="' + fid + '"></span>',
            );
      ...
      currentSendMethod: function ($elem) {
        var submitType = $elem
          .closest(BrainyFilter.filterFormId)
          .data("submit-type");
        var submitDelay = $elem
          .closest(BrainyFilter.filterFormId)
          .data("submit-delay");
        switch (submitType) {
          case "auto":
          ...
          case "delay":
            ...
            BrainyFilter.timeout = setTimeout(
              BrainyFilter.sendRequest,
              submitDelay,
            );
            break;
        ...
        if (BrainyFilter.ajaxEnabled) {
          ...
          this.ajaxHandler = $.ajax({
            url:
              ...
              BrainyFilter.baseUrl +
              "index.php" +
              BrainyFilter.prepareFilterData(
                false,
                "extension/module/brainyfilter/ajaxfilter",
              ) +
              "&count=" +
              (BrainyFilter.requestCount ? "1" : "0") +
              "&price=" +
              (BrainyFilter.requestPrice ? "1" : "0") +
              "&withContent=1" +
              "&curRoute=" +
            ...
            success: function (res) {
              ...
              var newUrl =
                window.location.origin +
                BrainyFilter.prepareFilterData(true, false);
              ...
              BrainyFilter.selectionCache =
                BrainyFilter.serializeMultipleForms();
          ...
          if (BrainyFilter.redirectTo) {
            newUrl =
              BrainyFilter.redirectTo +
              (BrainyFilter.redirectTo.match(/\?/) ? "&" : "?") +
              BrainyFilter.serializeFilterForm();
          } else {
            newUrl =
              window.location.origin + BrainyFilter.prepareFilterData(true);
          }
      ...
      updateProductList: function (html, bfData) {
        var $gridBtn = $("#grid-view").clone(true),
          ...
          $emptyMsg = $("<p/>", { class: "bf-empty-product-list-msg" }).html(
            bfLang.empty_list,
          );
        if ($prodCont.size()) {
        ...
        // --- НАЧАЛО ДОБАВЛЕННОГО КОДА ДЛЯ СЧЕТЧИКОВ ---
        var $newCounters = $html.find(".catalog_content-value");
        if ($newCounters.length) {
          $(".catalog_content-value").each(function (index) {
            if ($newCounters.eq(index).length) {
              $(this).html($newCounters.eq(index).html());
            }
          });
        } else if (!$prodCont.size()) {
          // Если товаров нет, показываем нули
          $(".catalog_content-value").html("Показано: 0 из 0");
        }
        ...
        try {
          ...
          } else if (typeof dataAnimate === "function") {
            //support for Boss Themes animation
            dataAnimate();
          } else {
            if (
          ...
          if (bfparam !== "") {
            this.each(function () {
              ...
              if (typeof url !== "undefined") {
                url = url.replace(
                  /[\?\&](bfilter|manufacturer_id|search|category_id)\=[^\&]+/g,
                  "",
                );
                url += /\?/.test(url) ? "&" : "?";
                url += bfilter;
      ...
      prepareFilterData: function (full, route) {
        ...
        query = window.location.search.replace(
          /[\&\?]+((route)|(path)|(page)|(bfilter)|(search)|(category_id)|(sub_category)|(description)|(manufacturer_id))[^&]+/g,
          "",
        );
        ...
        if (route) {
          ...
          form = form
            .replace(/[\&](route|path|manufacturer_id)=[^\&]+/g, "")
            .replace(/[\&]+$/, "");
          if (nPath !== "") {
        ...
        for (var id in filters) {
          if (filters.hasOwnProperty(id)) {
            if (!$.isArray(filters[id])) {
              bfilter +=
                id + ":" + filters[id].min + "-" + filters[id].max + ";";
            } else {
      ...
      changeTotalNumbers: function (data) {
        ...
        var $ctrls = $form
            ...
            .not(".bf-default"),
          $checkedBlks = $form
            .find(".bf-attr-filter input[type=checkbox]")
            .filter(":checked")
        ...
        $ctrls.each(function () {
          ...
          if (data[gid] && data[gid][val]) {
            if ($this.prop("tagName") === "OPTION") {
              ...
              var $row = $this
                .closest(".bf-attr-filter")
                .removeClass("bf-disabled");
              $row
                .find(".bf-cell")
                .last()
                .append('<span class="bf-count">' + data[gid][val] + "</span>");
            }
        ...
        $checkedBlks.find(".bf-disabled").each(function () {
          ...
          $this
            ...
            .find(".bf-cell")
            .last()
            .append('<span class="bf-count bf-empty">0</span>');
        ...
        $form.find("select").each(function () {
          if (
            $(this).find("option").not(".bf-default,[disabled]").size() === 0
          ) {
      ...
      initHorizontalScrolls: function () {
        var scroll = function (block, direction) {
          ...
          var curOffset =
            -parseInt($block.find(".bf-attr-block-cont").css("left")) || 0;
          ...
          $block
            .find(".bf-attr-block-cont")
            .stop()
            .animate({ left: "-" + newOffset + "px" }, 1200);
        };

        var filterRows = $(".bf-horizontal")
          .find(".bf-attr-block")
      ...
      floatSubmit: function () {
        ...
        var panel = $('<div class="bf-float-submit"></div>')
          .prepend(tick)
          .append(btn)
          .append(closeBtn);
        $("body").append(panel);
        ...
        var showBtn = function () {
          ...
          if (
            BrainyFilter.selectionCache === form ||
            cont.closest(".bf-responsive.bf-active").hasClass("bf-opened")
          ) {
            hideBtn();
            return;
          }
          ...
          var panelNewLeft =
            outBlockOffset.left + blockWidth + panelWidth > winWidth
      ...
      initSliding: function () {
        $(this.filterFormId).each(function () {
          ...
          $form.find(".bf-attr-block-cont").each(function (i, v) {
            ...
            if (enableSliding) {
              var count =
                $this.find(".bf-attr-filter").filter(":visible").size() -
                visibleItems;
      ...
      shrinkBlock: function (block, items, disableAnim) {
        ...
        var visibleItems = parseInt($form.data("visible-items"));
        var count =
          $(block).find(".bf-attr-filter").filter(":visible").size() -
          visibleItems;
        var height = 0;
        ...
        if (!items) {
          $(block)
            .closest(".bf-attr-block")
            .find(".bf-arrow")
            .css("background-position", "50% -128px");
        }
      ...
      expandBlock: function (block, disableAnim) {
        ...
        $(block)
          .parents(".bf-attr-block")
          .find(".bf-arrow")
          .css("background-position", "50% -153px");
      },
      ...
      initAbsolutePosition: function () {
        ...
        if ($layout.size()) {
          ...
          isHorizontal = $form
            .closest(".bf-check-position")
            .hasClass("bf-horizontal");
      ...
      priceSliderLabels: function ($slider) {
        var w = $slider.outerWidth(),
          ...
          d =
            px < 2
              ? w
              : ((BrainyFilter.max.toFixed(2) + "").length + 1) * 6 * 1.5,
          n = Math.floor(w / d),
          dx = w / n,
        ...
        for (var i = 1; i < n; i++) {
          var p = Math.round(
            BrainyFilter.min +
              ((i * dx) / w) * (BrainyFilter.max - BrainyFilter.min),
          );
...
jQuery(document).ready(function () {
  ...
  if (
    ...
    BrainyFilter &&
    !BrainyFilter.journalCompatibilityEnabled
  ) {
