...
$(document).ready(function () {
  ...
  var $searchInput = $(".header_search-inp");
  var $searchDropdown = $(".header_search-dropdown");
  var $searchItems = $(".header_search-items");
      ...
      success: function (json) {
        ...
        if (json.length > 0) {
          $.each(json, function (i, product) {
            ...
            var itemHtml = `
                            <a href="${product.href}" class="search_item">
                                <div class="search_item-img">
          ...
          // Добавляем кнопку "Смотреть все" в конец списка
          var seeAllUrl =
            "index.php?route=product/search&search=" + encodeURIComponent(v);
          $searchItems.append(`
                        <a href="${seeAllUrl}" class="header_search-btn btn">
                            Смотреть все <img src="catalog/view/theme/lydiiase/assets/img/link.svg" alt="">
                        </a>
                    `);
  ...
  update: function (key, quantity) {
    $.ajax({
      ...
      success: function (json) {
        // Need to set timeout otherwise it wont update the total
        setTimeout(function () {
          $("#cart-total-items").html(json["total"].match(/\d+/));
        }, 100);
