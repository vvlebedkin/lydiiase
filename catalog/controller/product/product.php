...
foreach ($this->session->data['recently_viewed'] as $recent_id) {
    ...
    if ($result) {
        ...
        $data['recently_viewed_products'][] = array(
            ...
            // Подгружаем модель информационных страниц
            $this->load->model('catalog/information');

            // Получаем данные для страницы "Доставка" (допустим ID 6)
            $delivery_info = $this->model_catalog_information->getInformation(8);
            if ($delivery_info) {
                $data['delivery_text'] = html_entity_decode($delivery_info['description'], ENT_QUOTES, 'UTF-8');
            } else {
                $data['delivery_text'] = '';
            }

            // Получаем данные для страницы "Оплата" (допустим ID 5)
            $payment_info = $this->model_catalog_information->getInformation(7);
            if ($payment_info) {
                $data['payment_text'] = html_entity_decode($payment_info['description'], ENT_QUOTES, 'UTF-8');
            } else {
                $data['payment_text'] = '';
            }
